...
[http://md.psych.bio.uni-goettingen.de/mv/unit/ggplot2/ggplot2.html](http://md.psych.bio.uni-goettingen.de/mv/unit/ggplot2/ggplot2.html)
...
Hier handelt es sich um eine , daher verwenden wir die Funktion `stat_smooth()`.
Die Syntax ist aber exakt die gleiche wie bei 'normalen' `geoms`:
```{r, warning=FALSE, message=FALSE}
aut_trade_plot <- aut_trade_plot +
  stat_smooth()
aut_trade_plot
...
Unschön hier ist nur der 'Rand', den `ggplot2` automatisch an den jeweiligen Enden der Skala
hinzufügt. 
Dieser Rand kann durch das Argument `expand` geändert werden.
Wie übergeben `expand` im einfachsten Falle einen Vektor mit zwei zahlen:
die erste Zahl bestimmt eine Konstante, die auf beiden Seiten zur Skala 
hinzuaddiert wird, die zweite Zahl einen Skalar der die Skala um den ensprechenden
...
`panel.grid.minor` für die Linien an den auf den Achsen aufgeschriebenen
Werten und dazwischen.^[Wenn Sie den horizontalen und vertikalen Grid
...
Auch dass die Achenlinien weg sind schaut nicht so schön aus.
Wir fügen Sie über das Argument `axis.line` mit der Funktion
`element_line()` explizit hinzu!

Sehr hässlich sind dagegen die kleinen schwarzen Zacken bei jedem Wert auf der
x- und y-Ache. 
...
Sie merken bereit: `theme()` können Sie quasi alles an Ihrer Grafik ändern was 
...
Gleichzeitig mag es auch nervig sein, so viele Einstellungen immer manuell
...
* expand scale einführen
* Transformation zu Prozent

### Farbenmanagement

### Style Guidelines

### ggplot Themen {#vis-theme}
