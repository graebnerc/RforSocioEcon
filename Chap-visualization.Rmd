...
<!-- Vielleicht hier jetzt nochmal eine Verlinkung einfügen, dass das folgende Kapitel eine Einführung in Regressionsanalysen bietet? -->
...
<!-- Irgendwie funktionieren in der HTML Version nur die Verlinkungen zu Kapitel 6.4 und 6.5 bei mir. -->
...
beschreiben. 
Hierbei wird kein besonderes R-Objekt erstellt, in dem die Grafik gespeichert wird - vielmehr speichern Sie am Ende ihr 'vollgemaltes Blatt' entweder als Bild ab, oder Sie verwerfen es und beschreiben ein neues 'Blatt'.
...
Thema `base` vs. `ggplot2` gesammelt und fasse mich hier daher kurz:
...
<!-- Irgendwie ist es manchmal capitalized, manchmal nicht. Ich würde es einfach jetzt mal überall capitalizen um hervorzuheben dass es ein Eigenname ist? -->
...
Dadurch ist auch die Vorgehensweise motiviert, zunächst ein Objekt mit der *Beschreibung* der Grafik zu erstellen und die Grafik dann am Ende durch Anwendung einer `print`-Funktion auf diese Beschreibung herzustellen. So können Sie das Objekt mit der Beschreibung vorher bereits speichern und weitergeben und dann zu einem späteren Zeitpunkt erst die eigentliche Grafik erstellen. 
...
Die Funktion `ggplot2::ggplot()` erstellt eine Liste, in der die Grafik-Spezifikationen gespeichert werden und akzeptiert über die Argumente `data` und `mapping` die Standard-Daten
...
Wenn Sie sich genauer mit der hierarchischen Grammatik beschäftigen wollen, die `ggplot2` zugrundeliegt, kann ich Ihnen den Originalartikel von @wickhamggplot empfehlen.
...
```{r, echo=FALSE}
ggplot2::ggsave(plot = offenheit_plot, 
...
<!-- Warum sind im Pdf hier so viele Leerzeichen zwischendurch immer? Muss ich darauf achten das zu formattieren? -->
...
<!-- Hier sind in der PDF wieder riesige Leerstellen irgendwie -->
...
`ggplot2::labs()` akzeptiert drei optionale Argumente: `title` für den Titel, `subtitle` für den Untertitel und `caption` für eine Fußnote, die sich besonders gut eignet um die Quelle der Daten anzugeben.
...
<!-- Generelle Frage: Müsste man nicht eigentlich bei der Quelle schreiben "Daten: Weltbank" statt "Quelle: Weltbank", weil die Grafik ja nicht von der Weltbank erstellt wurde? Oder ist das bei deskriptiven Statistiken egal bzw eh klar ob man sagt "Daten" oder "Quelle"? -->
...
<!-- Muss ich hier bei den einzelnen Unterargumenten auch immer jeweils ggplot2:: mit dazufügen? Das hab ich mich auch bei aes() gefragt.. -->

Sie merken bereits: mit `ggplot2::theme()` können Sie quasi alles an Ihrer Grafik ändern was 
...
die Funktion `icaeDesign::theme_icae()` verwendet werden. Um das Paket `icaeDesign` zu installieren müssen Sie folgendermaßen vorgehen:
...
ggplot2::ggsave(filename = here("output/trade_ts.pdf"), 
       plot = aut_trade_plot, 
       width = 9, 
       height = 6)
```
...
vollkommen ausreichend. Das geht über `ggplot2::theme()` mit `panel.border=element_blank()`. Dann sollten Sie allerdings die Achsen wieder mit `axis.line=element_line()` hinzufügen.
* Überlegen Sie sich gut ob Sie eine Legende brauchen und wo sie möglichst
platzsparend plaziert werden kann.
Innerhalb von `ggplot2::theme()` geht das über das Argument `legend.position`, welches für Legenden außerhalb des Plots `'top'`, `'bottom'`, `'left'` oder `'right'`, und für Legenden innerhalb des Plots die Koordinaten innerhalb des Plots mit `c(x, y)` akzeptiert. 
...
<!-- Hier ist irgendwie ein stilitischer Bruch vom Fließtext zu dieser Art von kurzen Übersichten. Soll das so sein oder wolltest du das noch ausformulieren? Oder vllt zumindest ein einleitender Satz hier zum Anfang bevor das "geeignet für" und "mgl Probleme" kommt? -->
...
<!-- Hier verstehe ich nicht ganz was das alpha=0.75 bei geom.point bedeutet? Gibt das die größe der Punkte an? Würde das kurz erwähnen. -->
...
<!-- Würde hier auch wieder zumindest mit einem Satz in das Beispiel einführen, bevor du die Daten zeigst -->
...
<!-- Hier evtl auch wieder ein Einleitungssatz -->
...
```
<!-- Hier kommt irgendwie eine Warnung -->
...
<!-- Stimmt es dass scale_color_icae aus icaeDesign kommt? -->
...
<!-- Kurz zur Info: diese Sätze standen ursprünglich weiter unten, habe die jetzt hier hochgezogen und den Satz mit "auch diese.." an dessen Stelle nach unten gesetzt. -->
...
<!-- Hier sind in der PDF wieder Warning nachrichten  -->
...
ggplot2::ggplot(data = exportzusammensetzung, 
  ...
  ggplot2::scale_color_icae(palette = "mixed", aesthetics = c("color", "fill")) +
  icaeDesign::theme_icae() +
...
<!-- Reorder ist im Paket stats, aber wenn ich das richtig verstehe ist das auch
eins der basic programme, die man nicht in einer library laden muss, korrekt? deshalb würde ich das jetzt hier nicht separat angeben. Oder soll ich "stats::reorder" schreiben?-->
...
<!-- An dieser Stelle kommt jetzt Output, der den Leser*innen an dieser Stelle noch nicht vertraut ist, ich denke mal weil das Regressionskapitel halt nach hinten verschoben wurde. Denkst du das ist ein Problem? Vllt nochmal eine Referenz auf das Regressionskapitel setzen? -->
...
<!-- Hier ist im Pdf output wieder ein weirder großer Absatz mit nichts -->
...
häufig verwendete `\` als `\\` schreiben, damit es in R als `\` interpretiert
wird:

<!-- Das müsste "in R" heißen hier, nicht "in LaTex", oder? Oder versteh ich das falsch? -->
...
### Mehr zu den Skalen: `ggplot2::expand_scale()` und Skalentransformation
...
In diesem Fall können wir die Hilfsfunktion `ggplot2::expand_scale()` verwenden.
...
ggplot2::scale_y_continuous(expand = expand_scale(mult = 0, add = 0))
```

Allerdings kann `ggplot2::expand_scale()` auch jeweils einen Vektor mit zwei Elementen
verarbeiten, wobei dann die erste Zahl für den unteren und die zweite für den
oberen Rand steht:

```{r, eval=FALSE}
ggplot2::scale_y_continuous(expand = expand_scale(mult = c(0, 0), add = c(0, 2))
```
...
<!-- Hier war ich ein wenig erstaunt über die "Farbe" #264062. Ist das einfach irgendeine Farbbezeichnung die du schön findest? Warum nicht einfach color="black"? Und: In der Pdf wechselt der Farbcode irgendwie zwischendrin, und die letzten 3 layer stehen da in grün, nicht in blau (ebenso wie die plusse in grün sind nicht mehr in rot. Hat das irgendwas zu bedeuten?) -->
...
<!-- Ist das hier korrekt, dass mutate aus dplyr kommt, und nicht aus ggpubr? -->
...
<!-- Hier frag ich mich wieder etwas warum so komplizierte Nummern bei den Farben -->
...
* Wir verwenden die Funktion `ggplot2::expand_scale()` wie oben eingeführt, da die 
...
#Hier kommt in der PDF irgendwie eine Warnung und viel leerer Platz zwischen
...
aber einfach die Achse zwischendrin 
[abzuschneiden](http://ksrowell.com/blog-visualizing-data/2013/08/12/how-to-simulate-a-broken-axis-value-axis/). 

<!-- Ich hab den Link gerade mal kontrolliert (mache ich bei allen, auch aus Eigeninteresse), und bei diesem hier kam eine Fehlermeldung, die Seite scheint nicht mehr zu existieren leider -->

Das Prinzip bleibt das gleiche und so etwas ist in keinem Fall eine gute Idee!
...
```
```{r, eval=FALSE, echo=FALSE}
data.table::fwrite(ameco_data, here::here("data/tidy/ameco_de.csv"))
```
```{r, echo=FALSE}
ameco_data <- data.table::fread( here::here("data/tidy/ameco_de.csv"), 
...
<!-- In der PDF scheint die Referenz nicht anständig zu funktionieren, und auch die 3 Bilder sind nicht nebeneinander gesetzt, aber zumindest die Referenz sieht hier richtig aus. Vllt hab ich noch eine zu alte Pdf und sollte eine neue machen? Ich lasse jetzt einfach erstmal den Kommtar hier, kannst ja nochmal nen Blick auf den Code werden ob ich was übersehe -->
...
knitr::include_graphics(here("figures/vis-faz-betrug.png"), auto_pdf = T)
knitr::include_graphics(here("figures/vis-hb-betrug.png"), auto_pdf = T)
```
...
befassen wollen:
Die `ggplot2` zugrundeliegende Idee einer *grammar of graphics* geht, wie anfangs des Kapitels schon beschrieben, auf 
