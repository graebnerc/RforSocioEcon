...
<!--@Birte: Bitte Verweise zu den Kapiteln setzen -->
...
Im Folgenden werden für beide Fälle wichtige Grundkonzepte eingeführt und kurz
erläutert.
...
pdf_plot <- ggplot(y_vals_pdf, 
  ...
  scale_y_continuous(expand = expansion(c(0,0), c(0, 0.1))) +
...
ggplot(data.frame(x = p_vals, y=qbinom(p_vals, anzahl_versuche, 0.25))) +
  ...
  ylab(TeX("Wahrscheinlichkeit $P(X\\leq x)$")) +
  xlab("Anzahl Erfolge (x)") +
  scale_y_continuous(expand = expansion(c(0, 0), c(0, 0.00))) + 
...
ggplot(data.frame(x=munzwurfe), aes(x=x)) +
  ...
  scale_x_continuous(breaks=seq(0, max(munzwurfe), 1)) +
  scale_y_continuous(expand = expansion(c(0,0), c(0,1))) +
  theme_icae()
```
...
ggplot(lambdas) +
  ...
  scale_y_continuous(expand = expansion(c(0,0), c(0,0.025))) +
  scale_x_continuous(expand = expansion(c(0,0), c(0,0))) +
  scale_color_icae(palette = "mixed", discrete = T) +
...
ggplot(lambda_4) +
  ...
  scale_fill_discrete(name = "New Legend Title") +
  scale_color_icae(palette = "mixed", discrete = T) +
  ggtitle(TeX("Die Poisson-Verteilung für $\\lambda = 4$")) +
  geom_segment(aes(x = x_val, y = 0, 
                   xend = x_val, yend = y_val),
               colour = "#800080") +
  geom_segment(aes(x = 0, y = y_val, 
...
### Grundkonzepte der Stochastik im stetigen Fall

Stochastische Unabhängigkeit im stetigen Fall bedeutet inhaltlich genau das
gleiche wie im diskreten Fall. Lediglich die Notation unterscheidet sich ein
wenig. 
Im diskreten Fall hatten wir angegeben:

$$.$$

Im stetigen Fall wird daraus:

$$.$$

Entsprechend ändert sich auch die Formulierung bedingter Wahrscheinlichkeiten.
Hatten wir im diskreten Fall noch geschrieben dass
...
Wenn wir den Satz von Bayes explizit für stetige ZV formulieren wollen verwenden
wir wieder die Dichte. 
Entsprechend wird aus der uns bekannten Formulierung

$$\mathbb{P}(A|B)=\frac{\mathbb{P}(A\cap B)}{\mathbb{P}(B)}=\frac{\mathbb{P}(B|A)\mathbb{P}(A)}{\mathbb{P}(B)}$$

die Folgende, wobei es sich bei $.$ um die Dichte

$$.$$

Schlussendlich wollen wir noch das Gesetz der totalen Wahrscheinlichkeit 
'übersetzen'. Aus
...
In all diesen Fällen ändert sich nichts an der Interpretation oder dem 
zugrundeliegenden Prinzip. 
Lediglich die Notation ist eine andere.
...
Und diese 'neuen' ZV sind genau das, womit wir uns hier noch zum Abschluss 
beschäftigen möchten.

Die Beispiele, die wir bisland kennen gelernt haben beschreiben alle die 
Verteilung einer einzelnen ZV beschäftigt.
Anhand den Konzepten der bedingten Wahrscheinlichkeit und der statistischen
