...
```{r, echo=FALSE}
pfad_bip_daten <- here("data/tidy/linmodels_bip_einleitung.csv")
```
...
data.table::fwrite(bip_daten, file = pfad_bip_daten)
```

```{r, echo=FALSE}
bip_daten <- data.table::fread(pfad_bip_daten)
```
...
```{r, echo=FALSE}
bip_plot <- ggplot2::ggplot(
  bip_daten, aes(y=Konsum, x=BIP)
  ) + 
  geom_point() + 
...
plot_file <- here::here(
  "figures/LineareReg-1/linreg1_bip-data.png")
save_pdf_png(
  filename = plot_file, plot = bip_plot, 
  width = 4, height = 3)
```
...
bip_plot_lm <- ggplot2::ggplot(
  bip_daten, aes(y=Konsum, x=BIP)
  ) + 
  ggplot2::geom_abline(
    intercept = schaetzung_bip[["coefficients"]][1],
    slope = schaetzung_bip[["coefficients"]][2], alpha=.5) +
  ggplot2::scale_x_continuous(
    limits = c(min(bip_daten$BIP), max(bip_daten$BIP))
    ) +
  ggplot2::scale_y_continuous(
    limits = c(min(bip_daten$Konsum), max(bip_daten$Konsum))
  ) +
  ggplot2::geom_point() + 
  ggplot2::annotate(
    geom = "text", hjust=0,
    label = TeX("Steigung der Geraden: $\\hat{\\beta}_1=0.71$"), 
    x=2.125, y=2.25) +
  ggplot2::ggtitle("Konsumausgaben und BIP") + 
  icaeDesign::theme_icae() 

plot_file <- here::here(
  "figures/LineareReg-1/linreg1_bip-lm.png")
save_pdf_png(
  filename = plot_file, plot = bip_plot_lm, 
  width = 4, height = 3)
```
...
ols_gerade_plot <- ggplot2::ggplot(
  datensatz, aes(x=x, y=y)) +
  ggplot2::geom_point() +
  ...
  scale_color_manual(
    values = c("Geschätzte Werte"=cols_used[1], 
               "Wahre Werte"=cols_used[2])
    ) + 
  ggplot2::geom_segment(
    aes(x = x[6], y = y[6], xend = x[6], yend=y[6]-fehler[6]),
    arrow = arrow(length = unit(0.03, "npc"), ends = "both"),
    colour = "#800080") +
  ggplot2::annotate(
    geom = "text", 
    label=TeX("$r_6=Y_6 - \\beta_0 - \\beta_1 x_6$"), 
    x = x[6]-0.2, y = y[6]-0.95, colour = "#800080") +
  icaeDesign::theme_icae() + 
  ggplot2::theme(legend.position = "none")

plot_file <- here::here(
  "figures/LineareReg-1/linreg1_ols-gerade.png")
save_pdf_png(
  filename = plot_file, plot = ols_gerade_plot, 
  width = 4, height = 3)
```
...
```{r, echo=FALSE, warning=FALSE}
compute_ols_plot <- ggplot2::ggplot(
  datensatz, aes(x=x, y=y)) +
  ggplot2::geom_point() +
  ggplot2::geom_abline(
    aes(color="Geschätzte Werte", 
        intercept = schaetzung[["coefficients"]][1], 
        slope = schaetzung[["coefficients"]][2]), 
    alpha=0.75, show.legend = TRUE, 
    color=get_icae_colors("dark blue")
    ) +
...
plot_file <- here::here(
  "figures/LineareReg-1/linreg1_compute-ols.png")
save_pdf_png(
  filename = plot_file, plot = compute_ols_plot, 
  width = 4, height = 3)
```
  ...
  ggplot2::annotate(geom = "text", 
    ...
    ggplot2::annotate(geom = "text", 
      ...
      ggplot2::annotate(geom = "text", 
           ...
           x = datensatz$x[2]+0.055, y = 3.25,
           colour = get_icae_colors("dark red")) +
      ggplot2::annotate(geom = "text", 
           label=TeX("$Y_3-\\bar{Y} = 1.4$"), 
           x = datensatz$x[3]-0.05, y = 4.45,
           colour = get_icae_colors("dark red")) +
      ggplot2::annotate(geom = "text", 
           label=TeX("$Y_4-\\bar{Y} = 0.02$"), 
           x = datensatz$x[4]-0.038, y = 3.9,
           colour = get_icae_colors("dark red")) +
      ggplot2::annotate(geom = "text", 
           label=TeX("$Y_5-\\bar{Y} = 0.22$"), 
           x = datensatz$x[5]-0.038, y = 4.1,
           colour = get_icae_colors("dark red")) + 
      ggplot2::annotate(geom = "text", 
           label=TeX("$TSS = 3.3$"), 
           x = 0.45, y = 3.0,
...
plot_file <- here::here(
  "figures/LineareReg-1/linreg1_tss-plot.png")
save_pdf_png(
  filename = plot_file, plot = tss_plot, 
  width = 6, height = 4)
```
...
```{r, echo=FALSE, warning=FALSE,}
rss_plot <- ggplot2::ggplot(datensatz, aes(x=x, y=y)) +
  ggplot2::geom_point() +
  ggplot2::geom_abline(aes(color="Geschätzte Werte", 
                  intercept = schaetzung[["coefficients"]][1], 
                  slope = schaetzung[["coefficients"]][2]), 
...
plot_file <- here::here(
  "figures/LineareReg-1/linreg1_rss-plot.png")
save_pdf_png(
  filename = plot_file, plot = rss_plot, 
  width = 6, height = 4)
```
...
ess_plot <- ggplot2::ggplot(
  ...
  ggplot2::scale_x_continuous(limits = c(0, 0.55), expand = c(0, 0)) +
    ...
    ggplot2::annotate(geom = "text",
           ...
           x = datensatz$x[1]-0.025, y = 3.25,
           colour = get_icae_colors("dark blue")) +
      ggplot2::annotate(geom = "text",
           label=TeX("$\\hat{Y}_2-\\bar{Y}$"),
           x = datensatz$x[2]-0.025, y = 3.4,
           colour = get_icae_colors("dark blue")) +
      ggplot2::annotate(geom = "text",
      ...
      ggplot2::annotate(geom = "text",
           ...
           x = datensatz$x[4]+0.025, y = 3.75,
           colour = get_icae_colors("dark blue")) +
      ggplot2::annotate(geom = "text",
           label=TeX("$\\hat{Y}_5-\\bar{Y}$"),
           x = datensatz$x[5]+0.025, y = 3.95,
           colour = get_icae_colors("dark blue")) +
      ggplot2::annotate(geom = "text",
...
plot_file <- here::here(
  "figures/LineareReg-1/linreg1_ess-plot.png")
save_pdf_png(
  filename = plot_file, plot = ess_plot, 
  width = 6, height = 4)
```
...
`r round(info_schaetzung[["r.squared"]], 2)*100` Prozent
...
beta_0_plot <- ggplot2::ggplot(
  ...
  ggplot2::annotate(geom = "text", label=TeX("$\\beta_0$"), 
                    x = beta_0_wahr-0.3, y=0.45) + 
  icaeDesign::theme_icae()
...
beta_plot <- ggpubr::ggarrange(beta_0_plot, beta_1_plot, ncol = 2)

plot_file <- here::here(
  "figures/LineareReg-1/linreg1_beta-plot.png")
save_pdf_png(
  filename = plot_file, plot = beta_plot, 
  width = 6, height = 4)
```
...
stichproben_plot <- ggplot2::ggplot(datensatz, aes(x=x, y=y)) +
  ggplot2::geom_point() +
  ggplot2::geom_abline(aes(color="Geschätzte Werte", 
                  intercept = gesch_b0, 
                  slope = gesch_b1), 
...
plot_file <- here::here(
  "figures/LineareReg-1/linreg1_stichproben-plot.png")
save_pdf_png(
  filename = plot_file, plot = stichproben_plot, 
  width = 6, height = 4)
```
...
plot_liste <- lapply(
  stichproben_n, plot_data, wahres_b0, wahres_b1)

full_plot <- ggpubr::ggarrange(
  plotlist = plot_liste, ncol = 2, nrow = 2, 
  common.legend = T, legend = "bottom")

full_plot <- ggpubr::annotate_figure(
  full_plot, 
  text_grob(TeX(paste0(
    "Regressionen mit $\\beta_0 = ",
    wahres_b0, "$ und $\\beta_1 = ", 
    wahres_b1, "$")), 
    face = "bold", size = 14))

plot_file <- here::here(
  "figures/LineareReg-1/linreg1_groessen-plot.png")
save_pdf_png(
