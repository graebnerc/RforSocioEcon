...
folgt, dann schreiben wir $P \propto B(n,p)$ und es gilt, dass 
...
Möchten wir schließlich eine bestimmte Menge an **Zufallszahlen** aus einer
Binomialverteilung ziehen geht das mit `rbinom()`:

```{r}
Zufallszahlen <- rbinom(n = 5, # Anzahl der Zufallszahlen
                        size = 10, # Parameter n
                        prob = 0.4 # Parameter p
                        )
Zufallszahlen
```
...
Möchten wir schließlich eine bestimmte Menge an **Zufallszahlen** aus einer
Poisson-Verteilung ziehen geht das mit `rpois()`:

```{r}
Zufallszahlen <- rpois(n = 5, # Anzahl der Zufallszahlen
...
Unter der **Standard-Normalverteilung** vertehen wir eine Normalverteilung mit
den Paramtern $\mu=0$ und $\sigma=1$.
...
### Theoretische und empirische Größen
