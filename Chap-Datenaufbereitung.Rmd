...
knitr::include_graphics(
  here::here("figures/Datenaufbereitung/Ablaufschema.png"), auto_pdf = T)
```
...
knitr::include_graphics(
  here("figures/Datenaufbereitung/Skalenniveaus.png"), auto_pdf = T)
```
...
export_data <- data.table::fread(
  here("data/raw/ComtradeExports.csv"), 
...
knitr::include_graphics(here("figures/Datenaufbereitung/world-bank.png"))
```
...
knitr::include_graphics(
  here::here("figures/Datenaufbereitung/raw-tidy.png"), auto_pdf = T)
```
...
```{r, echo=FALSE}
exports_unemp_data_tidy <- data.table::fread(
  here::here("data/tidy/wb_example.csv")
  ) %>%
  dplyr::rename(
    Land=iso2c, 
    Jahr=year,
...
exports_unemp_data_untidy_1 <- exports_unemp_data_tidy %>%
  dplyr::select(-Arbeitslosigkeit) %>%
  dplyr::filter(Jahr==2014) %>%
  tidyr::gather(Variable, value, -Land, -Jahr) %>%
  tidyr::spread(Jahr, value)
...
exports_unemp_data_untidy_3 <- exports_unemp_data_tidy %>%
  tidyr::gather(Variable, Wert, -Land, -Jahr) 
...
exports_unemp_data_tidy %>%
  dplyr::select(-Arbeitslosigkeit) %>%
  tidyr::gather(Variable, Werte, -Land, -Jahr) %>%
  tidyr::spread(Jahr, Werte)
```
...
data_al_exp <- data.table::fread(
  ...
  dplyr::select(-country) %>%
  tidyr::pivot_longer(
    cols = any_of(c("Exporte", "Arbeitslosigkeit")), 
    names_to = "Variable", 
    values_to = "Wert"
    ) %>%
  tidyr::pivot_wider(
    id_cols = any_of(c("Land", "Variable")), 
    names_from = "Jahr", 
    values_from = "Wert") 
data_al_exp
```
...
gini_data <- data.frame(
  Land=c("West Germany", "Germany", "West Germany", "Germany", "Germany"),
...
unemp_data_wb <- WDI::WDI(country = c("DE", "AT"), 
                          ...
                          start = 2010, end = 2016) %>%
  dplyr::rename(workforce_total=SL.TLF.TOTL.IN, # 1000 Persons
...
data.table::fwrite(unemp_data_wb, here("data/tidy/wb_unemployment.csv"))
```
...
unemp_data_wb <- data.table::fread(here("data/tidy/wb_unemployment.csv"), 
  ...
  dplyr::select(-country) %>%
  dplyr::rename(country=iso2c) %>%
  dplyr::select(any_of(c("country", "year", "laborforce_female", 
...
```{r, echo=F, eval=F}
macro_data <- readRDS(here("data/tidy/chap-data/comp_macro.rds")) %>%
  select(any_of(
    c("iso3c", "year", "AIS_mean", "AIS_mean_MIP", "AIS_mean_SGP", 
      "AIS_mean_Integrated", "country_group_jee"))
    ) %>%
  filter(
    !is.na(country_group_jee) & 
      !is.na(AIS_mean)& !is.na(AIS_mean_SGP) & !is.na(AIS_mean_MIP)
    ) %>%
  mutate(
    SGP_new=runif(21),
    SGP_avg=runif(21),
  ...
  rename(
    AIS_MIP=AIS_mean_MIP, 
    AIS_SGP=AIS_mean_SGP,
    AIS_INT=AIS_mean_Integrated,
    group=country_group_jee
  )
fwrite(macro_data, file = here::here("data/tidy/chap-data/eu_macro_ais.csv"))
```

Betrachten wie also folgendes Beispiel:

```{r, echo=F}
eu_ais_data <- data.table::fread(
  file = here::here("data/tidy/chap-data/eu_macro_ais.csv")) %>%
  dplyr::select(iso3c, year, group, dplyr::everything())
head(eu_ais_data, 3)
```
...
knitr::include_graphics(
  path = here::here("figures/Datenaufbereitung/Ordnerstruktur.png"), 
  auto_pdf = T)
```
