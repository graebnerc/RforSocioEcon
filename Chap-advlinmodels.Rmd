...
<!-- 2 und 4 sind irgendwie das gleiche oder? Frage 4 einfach löschen? -->
...
exo_full_plot <- ggpubr::ggarrange(exo_plot, exo_strict_plot, ncol = 2)
```
...
<!-- warum ist denn bei den figures in diesem kapitel der y- sowie x-achsenabschnitt nicht jeweils bei 0? wäre das nicht schöner? -->
...
<!-- Ich glaub diese graphik musst du mir nochmal besser erklären, die verstehe ich noch nicht. wie sehe ich denn in der rechten abbildunge dass der erwartungswert sich unterscheidet zwischen der unteren und oberen hälfte der beobachtungen? das sieht für mich alles ziemlich gleichmäßig aus oder?  -->
...
<!-- Doofe Frage aber gibt es nen Unterschied zwischen Multikollinearität und Kollinearität? (wenn nicht würde ich hier vllt einfach auch wieder "nicht perfekte Multikollinearität" schreiben einfach um zu zeigen dass der unterschied hier rein in perfekt vs nicht perfekt liegt, nicht in multikollinearität vs kollinearität) -->
...
<!-- Warum 2 abhängige Variablen? wollen wir nicht üblicherweise nur eine abhängige Variable erklären? -->
...
<!-- Ahh mmh ich kenn das gar nicht so dass hier jetzt auch nochmal sigma^2 und I steht, warum ist denn hier das I nochmal dabei wenn es nur darum geht auszudrücken dass die Fehlerterme Gaussian normalverteilt sind (das lässt sich doch einfach mit 0, sigma^2 ausdrücken)? -->
...
<!-- was heißt das genau, dass sie leicht abzuschwächen ist? -->
...
<!-- bin mir nicht sicher ob das hier richtig referenziert wird. wie wird ein theorem referenziert? hätte jetzt gedacht mit runden klammern und \@ref, oder? -->
...
immer genauer wird.
<!-- Aber haben wir das selbe nicht auch für die Erwartungstreue gesagt? Dass zwar für den einzelnen Fall wir nicht erwarten können, dass unser schätzer dem wahren beta Wert entspricht, asymptotisch aber schon? Siehe Zeile 472: "das Mittel unendlich vieler Schätzungen mit  -->
<!-- $\hat{\boldsymbol{\beta}}$ ist gleich $\beta$." Ich glaube deshalb bin ich verwirrt über den Unterschied zwischen Erwartungstreue und Konsistenz. -->
...
<!-- Erwartungstreue oder Konsistenz? In unserer metrics class gestern haben wir auch über einen ähnlichen trade-off geredet, aber da war der tradeoff zwischen effizienz und konsistenz, nicht zwischen effizienz und erwartungstreue. ich glaub ich bin einfach noch bissi verwirrt bzgl des unterschieds zwischen erwartunstreue und konsistenz, und auch warum es hier jetzt wieder um erwartungstreue geht während ja dieser ganze abschnitt hier eigl eher zum thema konsistenz ist oder? -->
...
<!-- ich glaub hier fehlt noch eine referenz in den klammern? -->
...
<!-- Warum nehmen wir hier standardabweichung nicht nicht varianz für die annahmenbeschreibung? es geht doch bei homoskedastie um konstante varianz, also eigl sigma squared oder?-->
...
```{r, echo=FALSE}
full_results <- data.table::fread(file = here("data/tidy/advlinmodels_full_results.csv"), 
                      colClasses = c(rep("double", 4), "character"))
```
<!-- Ich krieg hier eine Fehlermeldung wenn ich den chunk hier laufen lasse (hab das pdf noch nicht gecheckt). fehler in fread sagt es -->
...
```{r montecarlo, echo=FALSE, fig.cap="Vergleich der Standardfehler von $\hat{\boldsymbol{\beta}}$ bei Homoskedastie und Heteroskedastie."}
...
<!-- Hier muss das "vierte" Kapitel angepasst werden, und ich glaube aber auch dass das ja genau der Teil war der jetzt aus linmodels rausgeflogen ist, oder? Dh der müsste hier nochmal neu eingeführt werden?  -->
...
```{r, eval=FALSE, echo=FALSE}
fwrite(mcs_ergebnisse, 
       here("data/tidy/advlinmodels_mcs_ergebnisse_autoc.csv"))
```

```{r, echo=FALSE}
mcs_ergebnisse <- fread(here("data/tidy/advlinmodels_mcs_ergebnisse_autoc.csv"), 
                        colClasses = c(rep("double", 4), "character"))
```
...
tk_plot_small <- ggplot(
  ...
  ggtitle("TA-Plot (keine Autokorrelation)") +
  theme_icae()
...
tk_plot_mid <- ggplot(
  ...
  geom_point(alpha=0.5) +
  geom_hline(yintercept = 0) +
  ggtitle("TA-Plot (mittlere Autokorrelation)") +
  theme_icae()

tk_plot_large <- ggplot(
  data = tibble(
  ...
  geom_point(alpha=0.5) +
  geom_hline(yintercept = 0) +
  ggtitle("TA-Plot (starke Autokorrelation)") +
  theme_icae()

tk_plot_autoc <- ggarrange(tk_plot_small, tk_plot_mid, tk_plot_large, 
                           ncol = 3)
```
...
fwrite(mcs_full, file = here("data/tidy/advlinmodels_mcs_full.csv"))
```

```{r, echo=FALSE}
mcs_full <- fread(here("data/tidy/advlinmodels_mcs_full.csv"), 
...
fwrite(mcs_ovb, file = here("data/tidy/advlinmodels_mcs_ovb.csv"))
```


```{r, echo=FALSE}
mcs_ovb <- fread(here("data/tidy/advlinmodels_mcs_ovb.csv"), 
...
```{r, echo=FALSE}
wb_data <- fread(here("data/tidy/wb_bip_ineq_mort.csv"),
                 colClasses = c(rep("character", 2), rep("double", 5))) %>%
  filter(year==2000)
ggplot(data = wb_data, aes(x=GDP_PPPpc, y=MORTRATE)) +
...
```{r, echo=FALSE}
ggplot(data = wb_data, aes(x=log(GDP_PPPpc), y=log(MORTRATE))) +
  xlab("BIP pro Kopf (PPP, log)") + ylab("Kindersterblichkeit (log)") +
  geom_point() + theme_icae()
```
...
```{r, echo=FALSE}
chicken_daten <- fread(here("data/tidy/chicken_data.csv"), 
...
```{r, echo=FALSE}
chicken_nachfrage <- ggplot(data = chicken_daten, aes(x=p, y=q)) +
  xlab("Preis") + ylab("Konsum") + geom_point() + theme_icae() +
  geom_smooth(method = "lm", se = FALSE) +
  geom_abline() +
  ggtitle("Preis und Konsum von Hühnchenfleisch")

chicken_nachfrage_log <- ggplot(data = chicken_daten, aes(x=p, y=q)) +
  xlab("log(Preis)") + ylab("log(Konsum)") + geom_point() + theme_icae() +
  geom_smooth(method = "lm", se = FALSE) +
  scale_x_continuous(trans = "log") +
  scale_y_continuous(trans = "log") +
  ggtitle("Preis und Konsum von Hühnchenfleisch")

ggarrange(chicken_nachfrage, chicken_nachfrage_log, 
          ncol=2, nrow = 1)
```
