...
```{r nonlins, fig.cap="\\label{fig:nonlins}Lineare und nichtlineare Zusammenhänge.", echo=FALSE}
set.seed("123")
sample_size <- 50
...
lin_plot <- ggplot(data = full_sample, mapping = aes(x=x_2, y=y_lin)) +
  ggtitle("Linearer Zusammenhang") +
  geom_point() + theme_icae()

exp_plot <- ggplot(data = full_sample, mapping = aes(x=x_2, y=y_loglinear)) +
  ggtitle("Exp. Zusammenhang") +
  geom_point() + theme_icae()

log_plot <- ggplot(data = full_sample, mapping = aes(x=x_2, y=log(y_loglinear))) +
  ggtitle("Exp. Zusammenhang (log)") +
  geom_point() + theme_icae()
...
ggarrange(lin_plot, exp_plot, log_plot, nonlin_plot, 
          ncol = 2, nrow = 2, 
...
```{r, echo=FALSE}
full_results <- fread(file = here("data/tidy/advlinmodels_full_results.csv"), 
...
```{r, echo=FALSE}
ggarrange(tk_plot_homo, tk_plot_hetero, 
          ncol = 2, labels = c("a)", "b)"), 
