...
Nur wie genau produziert ein Schätzer einen Schätzwert für den wahren Wert $\theta$?
Wir unterscheiden grob zwischen zwei Methoden:
der **Momentenmethode** und der **Maximum-Likelihood Methode**.
...
zu rekonstruieren.
*Momente* korrespondieren dabei zu möglichen Kennzahlen, die uns 
interessieren.
Exakter definieren wir den $k$-ten Moment einer ZV $X$, $\mu_k(X)$, folgendermaßen:

$$\mu_k(X)=\mathbb{E}(X^k)$$

Damit ist unmittelbar ersichtlich, dass es sich beim ersten Moment um den 
Erwartungswert und beim zweiten Moment um die Varianz handelt. 

Die Momentenmethode macht sich nun zunutze, dass es für empirische Kennzahlen
immer ein theoretisches Gegenstück gibt. 
Für den Mittelwert als empirische Kennzahl wäre das z.B. der Erwartungswert der
zugrundeliegenden Verteilung.

Allgemeiner können wir die empirischen Momente definieren als:

$$m_k=\frac{1}{n}=\sum_{i=1}^nx_i^k$$

Die Schätzer wählt man also so, dass der empirische Moment gleich dem 
theoretischen Moment ist. 
Das ist eine in der regel recht einfach zu implementierende Aufgabe, allerdings
funktioniert sie nicht immer, und sie ist häufig nicht 'effizient' im Sinne, dass
sie den wahren Wert mit großer Genauigkeit angibt.
...
$\hat{\theta}$, so, dass die von uns beobachteten Daten das wahrscheinlichste 
...
Wir gehen also davon aus, dass $X\propto \mathcal{B}(n,p)$ und wissen, dass $n=50$.
...
$\mathbb{P}(X=40)=\binom{50}{40}p^40(1-p)^{50-40}$. Diese Funktion wird 
...
immer nur falsifiziert werden. Verifizieren können wir nichts!]
