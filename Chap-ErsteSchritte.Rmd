...
<!-- Der Output wird ja hier als #> [1] 5 gegeben. Macht es vielleicht Sinn hier noch einen Satz zu schreiben wie das zu lesen ist? steht #> immer vor jedem Output? (weiß ich gar nicht mehr). Und was genau sagt uns das [1] - einfach dass das unsere  erste Operation war? -->
...
<!-- "Daher" um die Wortwiederholung mit "Entsprechend" im Satz davor zu vermeiden -->
...
<!-- Hahaha möchtest du bei shit15_2 bleiben? :D Kann mir vorstellen dass wenn du es tatsächlich über einen publisher veröffentlichen willst, das evtl angekreidet wird. Vielleicht einen Namen setzen den du bei Studis schonmal gesehen hast, der vllt auf den ersten Blick gut scheint wenn man nicht so viel Erfahrung hat, aber der eigentlich zu lang oder zu wenig klar definiert ist? -->
...
## Zusammenfassung
<!-- Statt einer neuen Unterüberschrift würde ich hier einfach direkt unter den Code schreiben: "Zusammenfassend können wir sagen:" und dann die Bulletpoints. Das ist ja eigl nur eine Zusammenfassung von Unterkapitel 3.2, und eigentlich nicht unbedingt ein eigenes Unterkapitel 3.3 wert, oder?  -->
...
<!-- Etwas umformuliert weil ich über diesen Satz etwas gestolpert bin und ihn nicht  -->
<!-- so einfach zu lesen fand. -->
...
<!-- Frage die ich mir hier stelle: gibt es einen Grund dafür die Hypothenuse "1_hypothenuse" zu nennen statt einfach nur "hypothenuse"? Weshalb schreibt man den Code so? -->
...
werden, nach dem Funktionsaufruf verloren gehen:^[Das liegt daran, dass Funktionen 
ihr eigenes [environment](https://adv-r.hadley.nz/environments.html) haben.]
<!-- Das hier würde ich eigl sogar aus der Fußnote rausnehmen und hier als Halbsatz dranhängen: "...verloren gehen, weil Funktionen ihr eigenes environment haben. Deswegen..." -->
...
<!-- Ich verstehe hier noch nicht wie man TRUE oder FALSE in einen integer transformieren kann, und (wahrscheinlich noch wichtiger:) verstehe auch nicht komplett weshalb wir überhaupt transformieren wollen würden, also in welchem Kontext sowas hilfreich wäre. Vielleicht würde ich das noch irgendwo relativ am Anfang einführen, damit die Leser*Innen verstehen weshalb das hier eigentlich gerade alles erklärt wird? -->

<!-- PS: Nach dem Durcharbeiten dieses Kapitels habe ich nun verstanden wie TRUE und FALSE in integers transformiert werden (zu 0 und 1, wenn ich das richtig verstanden habe). Ich lasse den Kommentar oben dennoch stehen, als Info dass das zu diesem Zeitpunkt/an dieser Textstelle noch schwer verständlich war, falls du das als wichtig erachtest das hier dann schon zu erklären. Ist aber denke ich nicht super notwendig. Der Punkt mit dem praktischen Anwendungszweck von Transformationen ist allerdings immer noch eher offen für mich und wahrscheinlich auch wichtiger.  -->
...
fundamental als die bislang diskutierten Typen sind, jedoch häufig in der alltäglichen
...
<!-- Ist es hier wichtig genau zu verstehen was nochmal strings versus factors sind? Ich müsste das jetzt erstmal nochmal googlen (hab nur noch eine vage Erinnerung an den string Begriff und nicht sicher bei Factors, aber vielleicht ist das auch hier gar nicht so wichtig?) -->
...
<!-- Inhaltlich überhaupt nicht wichtig aber wäre es hier nicht intuitiver tatsächlich mögliche Körpergrößen bei "height" auszuwählen, also 189, 175, etc? -->
...
<!-- Zwar gibt es ein eigenes Kapitel zur Bearbeitung von Daten (Kap.5 hier verlinken?), wir wollen aber  -->
...
<!-- Ist @Packages etwas was man in RStudio aufrufen kann? Was ich googlen kann? Was genau mache ich hiermit? -->
...
In Kapitel 5 beschäftigen wir uns im Detail mit dem Lesen, Schreiben und Manipulieren von 
Datensätzen.
<!-- Sollte Kapitel 5 hier verlinkt werden? -->
