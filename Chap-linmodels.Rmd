...
```{r, echo=FALSE}
ggplot2::ggplot(bip_daten, aes(y=Konsum, x=BIP)) + geom_point() + 
...
ggplot2::ggplot(bip_daten, aes(y=Konsum, x=BIP)) + 
...
<!-- 
@Birte: ich habe diesen Teil nunmehr recht vollständig überarbeitet, bitte 
nochmal genau checken und vor allem Konsistenz mit dem späteren Test checken.
-->

Das lineare Regressionsmodell wird sehr häufig in der sozioökonomischen 
Forschung verwendet. 
Wie jedes statistisches Modell basiert es jedoch auf bestimmten Annahmen, 
...
unbeobachtbare G röße der Population treffen, nämlich die Fehlerterme 
...
Praktisch tritt dieser Fall, den man auch als 'perfekte Multikolinearität' 
bezeichnet, nur selten auf.
Würde tatsächlich perfekte Multikolinearität herrschen, wäre $\hat{\beta}$ 
...
nach dem GMT ist der OLS-Schätzer für lineare Modelle der beste konsistente 
Schätzer, den wir finden können. 
Oder cooler ausgedrückt: OLS ist der BLUE - der *Best Linear Unbiased Estimator*. 

Mit konsistent ist dabei gemeint, dass die Schätzer bei vielen Schätzversuchen
...
Daher sollte zu jeder Regressionsanalyse die Überprüfung der Annahmen 
dazugehören. 
Die notwendigen Methoden dazu lernen wir in Kapitel \@ref(advlin) kennen.

An dieser Stelle ist es noch einmal wichtig darauf hinzuweisen, dass wir eine 
Regression mit OLS schätzen können und keine Fehlermeldungen bekommen, obwohl 
