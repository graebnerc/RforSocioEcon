...
quart_differenz
```
<!-- Ich schlage hier im chunk vor die variable "quart-differenz" statt "quant-differenz" zu nennen damit klar ist dass das hier die Quartilsdifferenz ausgibt oder (auch wenn es dazu die quantile subtrahiert). Passt das? -->
...
```{r korrelation, fig.align='center', echo=FALSE}
c0_1a <- -50:50
c0_1b <- sapply(-50:50, function(x) x**2)
c0_1 <- ggplot2::ggplot(data.frame(x=c0_1a, 
                          y=c0_1b
                          ),
       aes(x=x, y=y)) +
  ggplot2::geom_point() +
  ggplot2::geom_smooth(method = "lm", se = FALSE, alpha=0.5) +
  ggplot2::ggtitle(paste0("Korrelation: ", 
                 round(cor(c0_1a, c0_1b, method = "pearson"), 2))) +
  ggplot2::theme_bw()

c0_2a <- -50:50
c0_2b <- sapply(-50:50, function(x) -x**2)
c0_2 <- ggplot2::ggplot(data.frame(x=c0_2a, 
                          y=c0_2b
                          ),
       aes(x=x, y=y)) +
  ggplot2::geom_point() +
...
c0_3a = data[, 1]  
c0_3b = data[, 2] 
c0_3 <- ggplot2::ggplot(data.frame(x=c0_3a, 
                          y=c0_3b
                          ),
       aes(x=x, y=y)) +
  ggplot2::geom_point() +
...
c0_4a = data[, 1]  
c0_4b = data[, 2] 
c0_4 <- ggplot2::ggplot(data.frame(x=c0_4a, 
                          y=c0_4b
                          ),
       aes(x=x, y=y)) +
  ggplot2::geom_point() +
...
c0_5 <- ggplot2::ggplot(data.frame(x=c0_5a, 
                          y=c0_5b
                          ),
       aes(x=x, y=y)) +
  ggplot2::geom_point() +
...
<!-- Discuss: Dieser Teil ist auch schon im Vizualizationskapitel drin (oder zumindest irgendwo in den letzten paar Kapiteln, erinner mich noch recht genau daran). Finde das Beispiel zwar sehr schön aber glaub das wäre vllt etwas zu viel das hier nochmal reinzusetzen? Außer halt wenn du wirklich davon ausgehst dass niemand das Buch von vorne bis hinten durchließt sondern sich immer nur einzelne Teile rauspickt.. Aber ansonsten vielleicht eher nochmal auf das Beispiel verweisen statt es nochmal wiederzugeben? -->

<!-- Nachtrag: weil noch nicht klar ob drinbleibt oder nicht habe ich jetzt figure und table noch nicht eingebettet beim beispiel -->
