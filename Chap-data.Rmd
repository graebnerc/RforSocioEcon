...
data_al_exp <- data.table::fread(here("data/tidy/wb_example.csv")) %>%
  dplyr::rename(Land=iso2c, 
         ...
         Arbeitslosigkeit=SL.UEM.TOTL.ZS) %>%
  dplyr::select(-country) %>%
  tidyr::pivot_longer(cols = one_of("Exporte", "Arbeitslosigkeit"), 
               names_to = "Variable", 
               values_to = "Wert") %>%
  tidyr::pivot_wider(id_cols = one_of("Land", "Variable"), 
              names_from = "Jahr", 
              values_from = "Wert") 
data_al_exp
```
...
data_al_exp_tidy <- data_al_exp %>%
  tidyr::pivot_longer(cols = -one_of("Land", "Variable"), 
               names_to = "Jahr", 
               values_to = "Wert") %>%
  tidyr::pivot_wider(id_cols = one_of("Land", "Jahr"), 
              values_from = "Wert", 
              names_from = "Variable")
data_al_exp_tidy
```
...
unemp_data_wb <- data.table::fread(here("data/tidy/wb_unemployment.csv"), 
  ...
  dplyr::select(one_of("country", "year", "laborforce_female", 
