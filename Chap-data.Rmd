...
Lauf [dieser Umfrage](https://whatsthebigdata.com/2016/05/01/data-scientists-spend-most-of-their-time-cleaning-data/)
verwenden Datenspezialisten regelmäßig 80\% ihrer Arbeitszeit auf Datenerhebung und
-aufbereitung.

Um bei diesem zentralen Schritt also Zeit und Nerven zu sparen ist es wichtig, 
...
Datenerhebung hin zum Forschungsergebnis transparent und nachvollziehbar ist.
...
ist entsprechend folgndermaßen strukturiert:
...
Die gröbste Unterscheidung kann zwischen **quantitativen** und **qualitativen**
...
Wirklich relevant ist jedoch die Unterscheidung der 
...
knitr::include_graphics("figures/data-data-classification.png")
```

TODO: EINZELNE NIVEAUS UND R BESCHREIBEN
...
[WDI](https://github.com/vincentarelbundock/WDI), welches Ihnen Zugriff
...
`read_dta()` aus dem Paket [haven](https://github.com/tidyverse/haven), 
...
aus dem Paket `R.utils` verwenden:
...
> **Beispiel für berühmte Menschen mit miserabler Datenaufbereitung: Der Reinhard-Rogoff Skandal**  
...
[tidyr](https://github.com/tidyverse/tidyr), welches auch Teil des `tidyverse` ist.^[
Die Funktionen `pivot_longer()` und `pivot_wider()` wurden in der neuesten Version
von `tidyr` eingeführt. Achten Sie also darauf, dass Sie die neueste Version
installiert haben. Sie ersetzen die Funktionen `spread()` und `gather()`, die
natürlich noch weiterhin funktionieren und die Sie in älterem Code sicher noch
häufig finden werden. 
...
Abschließend fassen wir noch die Funktionen in einer Tabelle zusammen:

| Funktion | Effekt | Reduduktion von Zeilen? | Duplikation von Zeilen? |
|----------+--------+-------------------------+-------------------------|
| `left_join()` | | Möglich |
| `right_join()` |  | Möglich |
| `inner_join()` |  | Möglich | Möglich |
| `full_join()` |  | ? | ? |
| `semi_join()` |  | ? | Nicht möglich |
| `nest_join()` |  | ? | ? | ? |
| `anti_join()` |  | ? | ? | ? |
...
Zweck werden wir hier die Funktionern `mutate()`, `summarise()` und
`group_by()` aus dem Paket `dplyr()` verwenden.
...
über dem aktuellen Wert zu bekommen.
