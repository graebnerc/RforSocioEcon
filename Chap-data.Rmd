...
<!-- Hier find ich den 3. Schritt im Code etwas schwieriger nachzuvollziehen, vielleicht könntest du da nochmal einen Satz zu schreiben? Laut dem obigen Satz klingt es so als seien nominale Daten immer beschreibbar als 'character' oder 'factor', aber laut dem Code sieht es so aus als sei ein Vektor nominaler Daten erstmal vom Typ 'character' und man müsste das dann erst mit dieser Funktion 'factor' umwandeln (allerdings wird es dann ja auch nicht in Faktoren umgewandelt sondern in Integer. Oder ist das das gleiche? Vielleicht ist mein Problem hier gerade auch noch dass du zu factors ja noch was schreiben wolltest bei Kapitel 3, ich weiß nicht ob das zusammengehört aber vielleicht ist das so ein bisschen der missing link? Ansonsten einfach hier nochmal einen Satz warum die diese Funktion factors(..) benutzt löst glaube ich die Sache auch schon.) -->
...
<!-- Mmmh aber hier hast du ja wieder die Funktion "factor(.., ..)" verwenden, nicht "as.factor(..)". Oder macht das das selbe? Wofür ist das "as." dann da wenn es eh keinen Unterschied macht? -->
...
<!-- Nur um zu prüfen ob ich das richtig verstehe: die Reihenfolge wird spezifiziert indem man im Vektor nicht mehr alle Daten einzeln eingibt, sondern wie oft in der Rangfolge angibt wie viele von den geordneten Kategorien je auftreten - also wenn ich c(rep(1,3), rep(3,6), rep (2,4)) schreiben würde, würde das ein Notensystem beschreiben indem die 3 vor der 2 geordnet ist. Ist das korrekt? -->
...
<!-- Hier würde ich nochmal einen kurzen Verweis auf den Anhang hinsetzen, in dem du ja ein Kapitel zu deskriptiver Statistik hast, da die statistischen Operationen hier ja nicht nochmal erklärt wurden -->
...
<!-- Den letzten Satz versteh ich noch irgendwie nicht so gut. Warum sollte man eine besonders ausgefallene Übersetzung durchführen wollen, und was heißt das? Dass man sich eine eigene Abkürzung ausdenkt? Was heißt es einen data.frame zu übergeben aber die Funktion ansonsten äquivalent zu nutzen? Dass jetzt einfach eine neue Übersetzungsart in der Funktion gespeichert ist? Wird dann die alte überschrieben? -->
...
```
```{r, echo=FALSE}
daten <- fread(here("data/tidy/wb_example.csv"))
```
```{r}
daten
```
...
<!-- Nur um zu kontrollieren ob ich das richtig verstehe warum die Datei jetzt schon in tidy liegt: Du öffnest eine Datei nicht aus dem Ordner raw und speicherst sie NACH Bearbeitung in tidy ab weil dann die Gefahr besteht das man ausverstehen falsch speichert und die raw Datei verändert hat. Richtig? Stattdessen legst du immer je eine Kopie der Daten in tidy ab, eine in raw, und öffnest dann die in tidy damit du sicher weißt dass an die raw Datei nichts drankommt. Frage nur extra nach weil ich irgendwie vorher den Eindruck hatte man öffnet schon einfach direkt Rohdateien als solche und speichert die danach einfach nur anders ab, aber wahrscheinlich macht es total viel Sinn von vorneherein eine Kopie im tidy Ordner zu erstellen -->
...
beschleunigt.
<!-- Welcher Lesevorgang wird beschleunigt wenn man Typen manuell spezifiziert und warum? -->
...
`fread()` verfügt noch über viele weitere Spezifizierungsmöglichkeiten, mit denen Sie
...
<!-- Beim Output steht bei BIP irgendwie die Nummern 1 und 2. Soll das so sein oder ist das ein Fehler? Ich versteh nicht was das heißen soll :D -->
...
<!-- An die head-Funktion erinner ich mich zwar noch aus meinem Kurs, ich glaub du hattest sie aber noch nicht im Skript erwähnt bisher (oder vielleicht im Kapitel 4, was ich aber ja jetzt erstmal übersprungen habe?). Vielleicht oben wenn du das "nrows" Argument für die fread Funktion erklärst, da irgendwo in der Ecke aus die head-Funktion erklären? -->
...
<!-- Im Vergleich zum Einlesen von Daten ist das Speichern (oder? Oder sollen Daten irgendwie geschrieben werden? Ich weiß nicht was das heißen würde, man schreibt doch die Daten nicht selber?) deutlich einfacher, weil -->
...
<!-- Okay nachdem ich jetzt unten weitergelesen habe wird der Begriff "schreiben" wohl verwendet um eine csv-Datei aus der Arbeit in R-Studio zu speichern/schreiben. Das wusste ich vorher nicht und bin im ersten Satz irgendwie darüber gestolpert und fand's verwirrend. Vielleicht oben noch erstmal von speichern reden und dann wird unten eh klar was damit gemeint ist? -->
...
<!-- Du hast Stata und das haven Paket oben schon verlinkt, also vielleicht nicht mehr extra nötig? Andererseits schadet es sicher auch nicht, falls jemand nur spezifisch zur Datenspeicherung etwas nachschlagen will und nur Kap.5.3.2 anschaut. Also: bemerke es hier nur damit du das auf dem Schirm hast, denke aber eigl du kannst die Verlinkungen drin lassen. -->
...
<!-- Mmmh okay das klingt jetzt doch wieder anders als oben, wo es so klang als sollte man zuerst eine Kopie der Rohdaten in den tidy Ordner legen und dann nur die Kopie öffnen, nicht die Rohdaten selber. Ist es egal wie herum man es macht oder gibt es da eine gängige best practice? -->
...
<!-- Hier fehlt irgendwie das Ende des Satzes -->
...
<!-- Das ist so spannend!! I had no idea! -->
...
```{r, echo=FALSE}
exports_unemp_data_tidy <- fread(here("data/tidy/wb_example.csv")) %>%
  rename(Land=iso2c, 
         Jahr=year,
...
<!-- Frage: Warum nimmst du im Code Jahr==2014 aber schreibst im Text von Jahr 2013? Unten schreibst du von "Spalte '2013'" aber momentan heißt die Spalte im Output 2014. -->
...
<!-- Wollen wir nicht eigentlich so gut wie immer mit tidy Daten arbeiten, wenn die meisten Statistik-Pakete und Funktionen das voraussetzen (so hatte ich das in Kapitel 5.4.1 verstanden)? Ich bin kurz zu dem verlinkten Kapitel 5.4.5 gesprungen und habe es überflogen aber beim Überfliegen jetzt noch keine Antwort gefunden (vllt hab ich zu schnell gelesen). Vielleicht wird das eh noch klar beim weiterlesen, aber ansonsten vllt hier ne Fußnote oder so dazusetzen (evtl statt der Verlinkung), in der beschrieben ist für welche Aufgaben welches Format besser ist?) -->
...
```{r, echo=FALSE}
data_al_exp <- fread(here("data/tidy/wb_example.csv")) %>%
  rename(Land=iso2c, 
         Jahr=year,
         Exporte=NE.EXP.GNFS.ZS, 
         Arbeitslosigkeit=SL.UEM.TOTL.ZS) %>%
...
<!-- Warum würde man das wollen? Dann sind doch genau die Daten aus dem anderen Datensatz verloren gegangen (Gini Daten)? In welchem Szenario würde sowas Sinn ergeben? -->
...
<!-- Ich glaube hier fehlt noch ein halbsatz oder? Was sehen wir dann? Der Code Output sagt mir irgendwie nicht so viel -->
...
gemeint ist, wie in den Argumenten von `*_join()`.
...
```{r, echo=FALSE}
unemp_data_wb <- fread(here("data/tidy/wb_unemployment.csv"), 
...
Es ist ein zentraler Nachteil von *point-and-click*-Software 
<!-- (gibt es hier ein Beispiel was so eine Softwäre wäre?) -->
...
<!-- vllt "neuen" statt "geupdateten" und den Anglizismus zu vermeiden? Oder meinste das geht klar? -->
...
```{r Ordnerstruktur-data, echo=FALSE, fig.align='center', fig.cap="Bewährte Ordnerstruktur für Forschungsprojekte mit R"}
