...
data_al_exp <- data.table::fread(here("data/tidy/wb_example.csv")) %>%
  dplyr::rename(Land=iso2c, 
         ...
         Arbeitslosigkeit=SL.UEM.TOTL.ZS) %>%
  dplyr::select(-country) %>%
  tidyr::pivot_longer(cols = any_of(c("Exporte", "Arbeitslosigkeit")), 
               names_to = "Variable", 
               values_to = "Wert") %>%
  tidyr::pivot_wider(id_cols = any_of(c("Land", "Variable")), 
              names_from = "Jahr", 
              values_from = "Wert") 
data_al_exp
```
...
data_al_exp_tidy <- data_al_exp %>%
  tidyr::pivot_longer(cols = -any_of(c("Land", "Variable")), 
               names_to = "Jahr", 
               values_to = "Wert") %>%
  tidyr::pivot_wider(id_cols = any_of(c("Land", "Jahr")), 
              values_from = "Wert", 
              names_from = "Variable")
data_al_exp_tidy
```
...
macro_data <- readRDS(here("data/tidy/chap-data/comp_macro.rds")) %>%
  select(any_of(c("iso3c", "year", "AIS_mean", "AIS_mean_MIP", "AIS_mean_SGP", 
...
fwrite(macro_data, file = here("data/tidy/chap-data/eu_macro_ais.csv"))
```
...
eu_ais_data <- data.table::fread(
  file = here("data/tidy/chap-data/eu_macro_ais.csv")) %>%
