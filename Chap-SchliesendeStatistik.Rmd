...
Binominalverteilung ist, dann lesen Sie nochmal das Kapitel \@ref(stat-stoch) zur Wahrscheinlichkeitstheorie.]
...
<!-- Verwendete Pakete auch wieder nach ganz oben setzen?  -->
...
<!-- Kommt das noch an späterer Stelle im Buch? Soll ich auf irgendein anderes Kapitel referenzieren? Oder rausnehmen? -->
...
genannt und später besprochen.]
<!-- Kapitel Referenz setzen? -->
...
<!-- fehlt hier noch was?bzgl der capital letters? -->
...
ggplot2::ggplot(data.frame(x = c(0, 100)), aes(x = x)) +
...
<!-- Ich frage mich ob sich das hier nicht widerspricht mit deinem Punkt oben in der fußnote um zeile 200 herum. Ich verstehe das so: wir können keine hypothesen verifizieren, aber (laut deinem punkt oben) die theorie schon bestätigen(verifizieren), indem wir die null-hypothese verwerfen. Dir gefällt das Wort "falsifizieren" nicht, weil das so klingt als würde es sich auf die theorie beziehen (statt nur eine hypothese zu verwerfen=zu falsifizieren). Ich glaube mein Punkt ist dass es scheint als würdest du verifizeren/bestätigen sowie falsifizieren/verwerfen unterschiedlich verwenden, mit verifizieren/bestätigen Bezug auf hypothese/theorie und falsifizieren/verwerfen Bezug auf theorie/hypothese. weiß nicht ob die Begrifflichkeit so allgemein verwendet wird? -->
...
ggplot2::ggplot(data.frame(x = c(0, 100)), aes(x = x)) +
  ...
  ggplot2::geom_segment(aes(x = 45, y = 0, 
               ...
               colour = "#800080") +
  ggplot2::geom_segment(aes(x = 45, y = 0.02, 
                   xend = 25, yend = 0.02),
               arrow = arrow(length = unit(0.03, "npc")),
               colour = "#800080") +
  ggplot2::geom_text(aes(x = 20, y =0.025), nudge_x = 0, nudge_y = 0.002,
            label = "Ablehnung wegen unwahrscheinlich\n weniger Erfolge",
            colour = "#800080") +
  ggplot2::geom_segment(aes(x = 55, y = 0, 
                   xend = 55, yend = dbinom(55, 100, 0.5)),
               colour = "#800080") +
  ggplot2::geom_segment(aes(x = 55, y = 0.02, 
                   xend = 75, yend = 0.02),
               arrow = arrow(length = unit(0.03, "npc")),
               colour = "#800080") +
  ggplot2::geom_text(aes(x = 80, y =0.025), nudge_x = 0, nudge_y = 0.002,
            label = "Ablehnung wegen unwahrscheinlich\n vieler Erfolge",
            colour = "#800080") +
  ggplot2::theme_bw()
```
...
<!-- Wie referenziere ich das? später im buch? einfach rausnehmen? -->
