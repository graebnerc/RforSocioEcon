...
lin_nonlin <- ggpubr::ggarrange(
  lin_plot, exp_plot, log_plot, nonlin_plot, 
  ncol = 2, nrow = 2, 
  labels = paste0(letters[1:4], ")")
  )

plot_file <- here::here(
  "figures/LineareReg-2/lin-nonlin-plot.png")
save_pdf_png(
  filename = plot_file, plot = lin_nonlin, 
  width = 8, height = 4.5)
```
...
exo_full_plot <- ggpubr::ggarrange(
  exo_strict_plot, exo_plot, ncol = 2, labels = c("a)", "b)"), 
  font.label = list(font="bold"))

plot_file <- here::here(
  "figures/LineareReg-2/exogeneity-plot.png")
save_pdf_png(
  filename = plot_file, plot = exo_full_plot, 
  width = 8, height = 4.5)
```
...
plot_file <- here::here(
  "figures/LineareReg-2/erwartungstreue-plot.png")
save_pdf_png(
  filename = plot_file, plot = erwartungstreue, 
  width = 6, height = 4)
```
...
effizienz <- ggplot2::ggplot(data = data_effizienz, 
  ...
  ggplot2::scale_color_manual(
    values = c("OLS"="#006600", "Ineffizient"="#800000"), 
    aesthetics = c("color", "fill"))

plot_file <- here::here(
  "figures/LineareReg-2/effizienz-plot.png")
save_pdf_png(
  filename = plot_file, plot = effizienz, 
  width = 6, height = 4)
```
...
```{r, echo=FALSE}
plot_file <- here::here(
  "figures/LineareReg-2/mcs_beta-1-plot.png")
save_pdf_png(
  filename = plot_file, plot = beta_1_plot, 
  width = 6, height = 4)
```

```{r montecarlo, fig.cap="Vergleich der Verteilung der mit Monte Carlo geschätzten Werte für $\\beta_1$ mit dem wahren Wert.", echo=FALSE}
knitr::include_graphics(plot_file, auto_pdf = T)
```
...
```{r, echo=FALSE}
plot_file <- here::here(
  "figures/LineareReg-2/mcs_beta-1-stdf-plot.png")
save_pdf_png(
  filename = plot_file, plot = beta_1_stdf_plot, 
  width = 6, height = 4)
```
...
```{r, echo=FALSE}
plot_file <- here::here(
  "figures/LineareReg-2/tk-plot.png")
save_pdf_png(
  filename = plot_file, plot = tk_full, 
  width = 6, height = 4)
```
...
```{r, echo=FALSE}
mcs_ergebnisse <- data.table::fread(
  here("data/tidy/advlinmodels_mcs_ergebnisse_autoc.csv"), 
  colClasses = c(rep("double", 4), "character"))
```

```{r, echo=FALSE, warning=FALSE}
b0_coef_plot <- ggplot2::ggplot(
  data = mcs_ergebnisse, 
  ...
  icaeDesign::scale_color_icae(
    palette = "mixed", reverse = T,
    aesthetics=c("color", "fill")
    ) +
  icaeDesign::theme_icae()

b1_coef_plot <- ggplot2::ggplot(
  data = mcs_ergebnisse, 
...
full_plot <- ggpubr::ggarrange(
  b0_coef_plot, b1_coef_plot, ncol = 2, 
  common.legend = T, legend = "bottom")
```

```{r, echo=FALSE}
plot_file <- here::here(
  "figures/LineareReg-2/mcs-autocor-plot.png")
save_pdf_png(
...
full_plot <- ggpubr::ggarrange(
  b0_stdf_plot, b1_stdf_plot, ncol = 2, 
  common.legend = T, legend = "bottom")
```

```{r, echo=FALSE}
plot_file <- here::here(
  "figures/LineareReg-2/mcs-autocor-stdf-plot.png")
save_pdf_png(
  filename = plot_file, plot = full_plot, 
  width = 6, height = 4)
```
...
tk_plot_autoc <- ggpubr::ggarrange(
  tk_plot_small, tk_plot_mid, tk_plot_large, ncol = 3)
```

```{r, echo=FALSE}
plot_file <- here::here(
  "figures/LineareReg-2/mcs-autocor-tk-plot.png")
save_pdf_png(
  filename = plot_file, plot = tk_plot_autoc, 
  width = 8, height = 4)
```
...
```{r, echo=FALSE}
mcs_full <- data.table::fread(
  here("data/tidy/advlinmodels_mcs_full.csv"), 
...
b0_mc <- ggplot2::ggplot(data = mcs_full, 
  ...
  icaeDesign::scale_fill_icae(palette = "mixed") +
  icaeDesign::theme_icae()
...
```{r, echo=FALSE}
plot_file <- here::here(
  "figures/LineareReg-2/mcs-dist-estimator.png")
save_pdf_png(
  filename = plot_file, plot = full_plot, 
  width = 8, height = 4)
```
...
```{r, echo=FALSE}
mcs_ovb <- data.table::fread(
  here("data/tidy/advlinmodels_mcs_ovb.csv"), 
...
full_plot <- ggpubr::ggarrange(
  b0_mc, b1_mc, ncol = 2, common.legend = T, legend = "none")
```

```{r, echo=FALSE}
plot_file <- here::here(
  "figures/LineareReg-2/mcs-omitted-var-bias.png")
save_pdf_png(
  filename = plot_file, plot = full_plot, 
  width = 8, height = 4)
```
...
```{r, echo=FALSE}
bipkonsum <- data.table::fread(
  here("data/tidy/BIPKonsum.csv"), 
  colClasses = rep("double", 3))

lin_bip <- ggplot2::ggplot(
  data = bipkonsum, aes(x=BIP, y=Konsumausgaben)
  ) +
...
```{r, echo=FALSE}
plot_file <- here::here(
  "figures/LineareReg-2/lin-bip.png")
save_pdf_png(
  filename = plot_file, plot = lin_bip, 
  width = 6, height = 4)
```
...
```{r, echo=FALSE}
wb_data <- data.table::fread(
  here("data/tidy/wb_bip_ineq_mort.csv"),
  colClasses = c(rep("character", 2), rep("double", 5))
  ) %>%
  dplyr::filter(year==2000)

nonlin_bip <- ggplot2::ggplot(
...
```{r, echo=FALSE}
plot_file <- here::here(
  "figures/LineareReg-2/nonlin-bip.png")
save_pdf_png(
  filename = plot_file, plot = nonlin_bip, 
  width = 6, height = 4)
```
...
```{r, echo=FALSE}
nonlin_log_bip <- ggplot2::ggplot(
  data = wb_data, aes(x=log(GDP_PPPpc), y=log(MORTRATE))
  ) +
...
```{r, echo=FALSE}
plot_file <- here::here(
  "figures/LineareReg-2/nonlin-log-bip.png")
save_pdf_png(
  filename = plot_file, plot = nonlin_log_bip, 
  width = 6, height = 4)
```
...
data.table::fwrite(
  x = selbstmord_daten, file = here::here("data/tidy/selbstmord_daten.csv"))
```

```{r, echo=FALSE}
selbstmord_daten <- data.table::fread(
  here::here("data/tidy/selbstmord_daten.csv"))
```
...
durkheimraw_full <- ggpubr::ggarrange(
  selbstmord_daten_plot, 
  selbstmord_daten_plot_log, ncol = 2)

plot_file <- here::here(
  "figures/LineareReg-2/durkheim_raw.png")
save_pdf_png(
  filename = plot_file, plot = durkheimraw_full, 
  width = 6, height = 3)
```

```{r durkheimraw, echo=FALSE, fig.cap="Simulierte Daten für Durkheim-Beispiel zum Zusammenhang zwischen sozialem Zusammenhalt und der Wahrscheinlichkeit zum Selbstmord."}
knitr::include_graphics(plot_file, auto_pdf = T)
```
...
durkheimraw_reg <- ggarrange(
  bias_komplettes_modell, bias_komplettes_modell_TA, ncol = 2
)

plot_file <- here::here(
  "figures/LineareReg-2/durkheim_reg.png")
save_pdf_png(
  filename = plot_file, plot = durkheimraw_reg, 
  width = 6, height = 3)
```

```{r durkheimreg, echo=FALSE, fig.cap="Die Schätzung eines linearen Modells auf die simulierten Durkheim-Daten ist eindeutig fehlspezifiziert."}
knitr::include_graphics(plot_file, auto_pdf = T)
```
...
plot_file <- here::here(
  "figures/LineareReg-2/durkheim_sep.png")
save_pdf_png(
  filename = plot_file, plot = getrennt_full, 
  width = 6, height = 3.5)
```

```{r durkheimsep, echo=FALSE, fig.cap="Auch eine Aufsplittung der Samples führt nicht dazu, dass der nicht-lineare Zusammenhang richtig geschätzt wird."}
knitr::include_graphics(plot_file, auto_pdf = T)
```
...
plot_file <- here::here(
  "figures/LineareReg-2/durkheim_quad.png")
save_pdf_png(
  filename = plot_file, plot = plot_quadratisches_modell_full, 
  width = 6, height = 3.5)
```

```{r durkheimquad, echo=FALSE, fig.cap="Durch das Hinzufügen eines quadratischen Terms in die Schätzgleichung kann das Modell korrekt spezifiziert und geschätzt werden."}
knitr::include_graphics(plot_file, auto_pdf = T)
```
...
```{r, echo=FALSE}
journal_daten <- data.table::fread(
  here("data/tidy/journaldaten.csv"), 
...
```{r, echo=FALSE}
journal_nonlin <- ggplot2::ggplot(data = journal_daten, 
...
```{r, echo=FALSE}
plot_file <- here::here(
  "figures/LineareReg-2/journal_nonlin.png")
save_pdf_png(
  filename = plot_file, plot = journal_nonlin, 
  width = 6, height = 4)
```
...
```{r, echo=FALSE}
journal_nonlin_log <- ggplot2::ggplot(
  data = journal_daten, 
...
```{r, echo=FALSE}
plot_file <- here::here(
  "figures/LineareReg-2/journal_nonlin_log.png")
save_pdf_png(
  filename = plot_file, plot = journal_nonlin_log, 
  width = 6, height = 4)
```
...
chicken_full <- ggpubr::ggarrange(
  chicken_nachfrage, chicken_nachfrage_log, 
  ncol=2, nrow = 1)

chicken_full <- ggpubr::annotate_figure(
  chicken_full, top = "Preis und Konsum von Hühnchenfleisch")
```

```{r, echo=FALSE}
plot_file <- here::here(
  "figures/LineareReg-2/chicken.png")
save_pdf_png(
  filename = plot_file, plot = chicken_full, 
  width = 6, height = 4)
```
...
```{r, echo=FALSE}
set.seed("123")
residuals_from_reg1 <- rnorm(2000)
residuals_from_reg2 <- rlogis(2000)
```
...
```{r, echo=FALSE}
qq_plot1 <- ggplot2::ggplot(
  data = tibble::tibble(
    Residuen=residuals_from_reg1
    ),
  mapping = aes(sample=Residuen)) + 
  labs(title = "Normalverteilte Residuen",
...
qq_plot2 <- ggplot2::ggplot(
  data = tibble::tibble(
    Residuen=residuals_from_reg2
    ),
  mapping = aes(sample=Residuen)) + 
  labs(title = "Nicht-normalverteilte Residuen",
       x = "Theoretische Quantile", 
       y = "Stichproben-Quantile") +
  ggplot2::stat_qq() + ggplot2::stat_qq_line() +
  theme_bw()+
...
qq_plot <- ggpubr::ggarrange(
  qq_plot1, qq_plot2, ncol = 2, labels = c("a)", "b)"), 
  font.label = list(font="bold")
)
```
...
```{r, echo=FALSE}
plot_file <- here::here(
  "figures/LineareReg-2/qqplot.png")
save_pdf_png(
  filename = plot_file, plot = qq_plot, 
  width = 8, height = 4)
```
