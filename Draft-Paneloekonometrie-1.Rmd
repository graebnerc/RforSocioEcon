...
Wir k\"onnen das nun konkret auf einen mit unserer Funktion oben produzierten Beispieldatensatz anwenden:
...
### Drei Schätzer mit fixen Effekten
...
1. Der **Pooling**-Schätzer $\hat{\beta}$, bei dem wir OLS unmittelbar auf
die Rohdaten anwenden:
\begin{align}
  \label{eq:ols-pooled}
  \hat{\beta}_{pooled}= (\boldsymbol{X'X})^{-1}\boldsymbol{X'y}
\end{align}
...
2. Der **Between**-Schätzer $\hat{\gamma}_{between}$, bei dem wir OLS auf
die mit der Matrix $B$ multiplizierten Rohdaten anwenden:
\begin{align}
  \label{eq:ols-between}
  \hat{\gamma}_{between}= (\boldsymbol{Z'BZ})^{-1}\boldsymbol{Z'By}
\end{align}
oder
\begin{align}
  \label{eq:ols-between2}
  \hat{\beta}_{between}= (\boldsymbol{X'\bar{B}X})^{-1}\boldsymbol{Z'\bar{B}y}
\end{align}
mit $\bar{B}=B-\boldsymbol{1}$.
3. Der **Within**-Schätzer $\hat{\gamma}_{between}$, bei dem wir OLS auf
die mit der Matrix $W$ multiplizierten Rohdaten anwenden:
\begin{align}
  \label{eq:ols-within}
  \hat{\beta}_{within}= (\boldsymbol{X'WX})^{-1}\boldsymbol{X'Wy}
\end{align}

Meist ist letzterer gemeint, wenn wir von einem *Fixed-Effects*-Schätzer
